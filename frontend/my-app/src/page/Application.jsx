...
import { Search, Calendar, MapPin, DollarSign, Mail, Phone, User, Clock, Briefcase, ChevronRight } from "lucide-react";
...
const Applications = () => {
        ...
        const fetchApplications = async () => {
            try {
                ...
                if (user.role !== "Job Poster") {
                    console.error("User is not a Job Poster. Role:", user.role);
                    setError("Only job posters can view applications.");
                    toast.error("Only job posters can view applications.");
                    setLoading(false);
                    return;
                }

                console.log("Fetching applications for employer:", user._id);
                const response = await axios.get("http://localhost:3001/api/job-applications/my-jobs", {
                    headers: {
                ...
                const errorMsg = error.response?.data?.error || "Failed to fetch applications.";
                setError(errorMsg);
    ...
    const handleStatusUpdate = async (applicationId, status) => {
        try {
            ...
            const response = await axios.patch(
                `http://localhost:3001/api/job-applications/${applicationId}/status`,
                { status },
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
    ...
    const handleDelete = async (applicationId) => {
        try {
            const token = localStorage.getItem("token");
            await axios.delete(`http://localhost:3001/api/job-applications/${applicationId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-blue-600 py-16">
                <div className="container mx-auto px-4 text-center text-white">
                    ...
                    <p className="text-lg opacity-90 max-w-2xl mx-auto">
                        Manage and track applications for jobs you posted
                    </p>
                </div>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-center">
                        <div className="relative flex-grow">
                            ...
                            <input
                                ...
                                className="pl-10 w-full border border-gray-200 rounded-lg py-2.5 px-4 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                value={searchTerm}
                        ...
                        <div className="flex gap-2 w-full md:w-auto">
                            <select
                                className="border border-gray-200 rounded-lg py-2.5 px-4 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                            ...
                            <button className="px-4 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center gap-2">
                                <Briefcase size={16} />
                                <span>Export</span>
                            </button>
                        </div>
                                            ...
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <div className="flex items-center gap-3">
                                                        <h3 className="text-lg font-bold text-gray-900">{application.fullName || "N/A"}</h3>
                                                        {application.status && (
                                                            <span
                                                    ...
                                                    <p className="text-sm text-gray-500 mt-1">
                                                        {application.jobId
                                                            ? `${application.jobId.title || "N/A"} at ${application.jobId.company || "N/A"}`
                                            ...
                                            <div className="mt-6 space-y-3">
                                                ...
                                                <div className="flex items-start">
                                                    ...
                                                    <span className="text-sm text-gray-600">
                                                        {application.jobId?.location || application.location || "N/A"}
                                                    </span>
                                                </div>
                                                <div className="flex items-start">
                                                    <DollarSign size={16} className="flex-shrink-0 mt-1 mr-2 text-gray-400" />
                                                    <span className="text-sm text-gray-600">
                                                        {application.expectedSalary ? `₹${application.expectedSalary}` : "Salary not specified"}
                                                    </span>
                                                </div>
                                                <div className="flex items-start">
